...
Version 1.123 restores direct Google Drive folder selection without restoring broad Drive access. Member authorization requests only Google's non-sensitive `drive.file` scope, the member explicitly chooses a folder, and Sodium automatically shares that selected folder read-only with its counting service identity. Web uses Google Picker; native iPhone uses Google's system-browser folder picker and returns through `sodium://drive`. Manual HTTPS links remain available. This release also separates existing-member login from one-time invite redemption so a consumed invite retained in a browser cannot send a valid member back into onboarding.
